# ChIP-seq analysis with R and Bioconductor {#chipseq}
...
## Introduction into ChIP-seq

One of the most fascinating biological phenomenons is the fact that a myriad of 
different cell types, in a multicellular organisms, are encoded
by one single genome. How exactly this is achieved is still a
...
Gene expression is controlled by a special class of genes called 
...
factors. Histone complexes can be chemically modified with different posttranslational modifications
(phosphorylation, methylation, acetylation...). Such modifications change histone
...
ChIP-seq stands for chromatin immunoprecipitation followed by sequencing.
It is an experimental method for finding locations on DNA which are bound
by proteins. It has been extensively used to study 
in-vivo binding preferences of transcription factors, and genomic distribution
of modified histones.

In the following tutorial you will learn how to assess quality control
...
# fetch the chromosome lenghts for the human genome
...
cpm = counts*(100000/colSums(counts))
```
...
```{r sample_clustering.complex_heatmap, fig.cap='Heatmap showing ChIP-seq sample similarity using Pearson correlation coefficient'}
# load ComplexHeatmap
...
```{r genome_browser.gviz_show, fig.cap='ChIP-seq signal visualized as a browser track using Gviz'}
...
\ref(fig:Figure_JaccardSimilarity).



```{r Figure_JaccardSimilarity, echo=FALSE, include=TRUE, fig.cap='Jaccard similarity is defined as the ration of the intersection and union of two sets.'}
...
#### Constructing genomic annotaion
...
```{r read_annot.plot, fig.cap='ChIP-seq signal distribution over functional genomic features'}
...
```{r chip_type.annotation, message = FALSE, error=FALSE}
library(AnnotationHub)
hub = AnnotationHub()
gtf = hub[['AH61126']]
...
```{r chip_type.plot_gviz, fig.cap='ChIP-seq signal around the URB1 gene'}
...
```{r peak_calling.sharp.plot, message = FALSE, erroe=FALSE, fig.cap='Comparison of CPM values between ChIP and Input experiments. Good ChIP experiments should always show enrichment'}
library(ggplot2)
...
```{r peak_calling.sharp.peak_calling.plot, fig.cap='Comparison of signal between ChIP and input samples. Red labeled dots correspond to called peaks.'}
library(ggplot2)
...
Using Gvis, we will construct the layered tracks.
...
```{r peak_calling.signal_profile.plot, fig.cap='ChIP and Input signal profile in around the peak centers.'}
...
it is important to compare the scales on both samples. The ChIP contains up 
to 250 reads, while the maximum value in the input sample is only 50 reads.
...
```{r peak_calling.broad.gviz, fig.cap='Visualization of H3K36me3 ChIP signal on a called broad peak.'}
...
```{r peak_quality.counts.plot, fig.cap='Percentage of ChIP read in called peaks. Higher percentage indicates higher ChIP quality.'}
...
```{r peak_quality.seqLogo, fig.cap="CTCF sequence motif visualized as a sequence logo. Y axis ranges from zero to two, and corresponds to the amount of information each base in the motif contributes to the overall motif. The larger the letter the greater the probability of observing just one defined base on the designated position."}
...
```{r, peak_quality.scan.dist.plot, fig.cap="Percentage of peaks containing the motif'. Higher percentage indicates a better ChIP-experiment, and a better peak calling procedure."}
...
```{r chip_quality.motifloc.plot, fig.cap='Transcription factor sequence motif localization with respect to the defined binding sites.'}
...
```{r peak_annotation.plot, fig.cap='Enrichment of transcription factor or histone modifications in functional genomic features'}
