...
```{r,fastq,fig.cap="Per base sequence quality boxplot",fig.align = 'center',out.width='80%',echo=TRUE, warning=FALSE}
...
writeFastq(fq[qcount == 0], 
           paste(fastqFiles[1], "Qfiltered", sep="_"))
```
...
f <- FastqStreamer(fastqFiles[1],readerBlockSize=1000) 
...
The read mapping in R can be done with gmapR` [@gmapR], `QuasR` [@gaidatzis_quasr:_2015], `Rsubread` [@liao_subread_2013], and `systemPipeR` [@backman_systempiper:_2016] packages. We will demonstrate read mapping with QuasR which uses **Rbowtie** package, which wraps the Bowtie aligner. Below, we show how to map reads from a ChIP-seq experiment using QuasR/bowtie. The interface is mostly simplified and it takes care of creating the index from a genome fasta file if there is no index provided. If you use bowtie from command line, the interface will not be as simple.
...
It is good to explain, what is going on here as the `qAlign` function makes things look simple. This function is designed to be easy. For example, it creates a genome index automatically if it does not exist if you provide a fasta file. We provided only two arguments for the fastq file and a reference genome file. In fact, this function also has many knobs and you can change its behavior by giving different arguments to affect the behaviors of the aligner Bowtie. For example, you can supply parameters to Bowtie using `alignmentParameter` argument, however the `qAlign()` function is optimized for different types of alignment problems and selects alignment parameters automatically. It is designed to work with alignment and quantification tasks for RNA-seq, ChIP-seq, small-RNA sequencing, Bisulfite sequencing (DNA methylation) and allele specific analysis. If you want to change default bowtie parameters only do it for simple alignment problems such as ChIP-seq and RNA-seq.
