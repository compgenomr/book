...
```{r Figure_ChIP-seq_Protocol, echo=FALSE, include=TRUE, fig.cap='Main experimental steps in the ChIP-seq protocol'}
...
The data sets are located in the following folder:
...
```{r correlation.plot, fig.width = 5, fig.height=3}
library(ggplot2)
...
```{r gc.plot, fig.width = 5, fig.height=3}
...
query(hub, c('ENSEMBL','Homo','GRCh38','chr','gtf'))
```
...
```{r read_annot.plot, fig.width = 6, fig.height=4}
...
```{r peak_calling.sharp.plot, fig.width = 4, fig.height=4, message = FALSE, erroe=FALSE}
library(ggplot2)
...
```{r peak_calling.sharp.peak_calling.plot, fig.width=6, fig.height=4}
library(ggplot2)
...
```{r peak_quality.counts.plot, fig.width = 5, fig.height=3}
...
```{r peak_quality.seqLogo, fig.width=7, fig.height=5}
...
```{r, peak_quality.scan.scan.dist, fig.width = 5, fig.height=3}
...
```{r chip_quality.motifloc.plot, fig.height=3, fig.width=5}
...
```{r peak_annotation.plot, fig.width = 5, fig.height=3}
...
```{r motif_discovery.seqlogo, fig.width = 3, fig.width = 6}
...
Therefore, it is customary to integrate our results data which is already available
for our system of interest - ChIP-seq of different proteins, genome wide measurements of 
expression, or assay for 3D genome structure.
...
With an increase in number of ChIP experiment, pairwise comparisons start being
