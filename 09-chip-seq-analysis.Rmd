...
From \@ref(processingReads) you should be familiar with \index{read filtering}
...
BAM\index{BAM file}, BED\index{BED file}, wig\index{wig file}, bigWig\index{bigWig file} ...
...
which was introduced in chapter \@(genomicIntervals).
...
```{r genome-browser-gviz-show, fig.cap='ChIP-seq signal visualized as a browser track using Gviz', width=8, height = 3}
...
If all of the windows do not have the same length (i.e. when at the ChIP-seq peaks), 
then the normalization is a prerequisite.
...
Figure \@ref(fig:gc-plot) visualizes the CPM Vs GC content, and
...
# gather converts a fat data.frame into a tall data.frame, 
...
Now we can visualize the relationship using  a scatterplot.
...
\index{transcription start site (TSS)}, exon, intron, intergenic, 

with the following hierarchy:

**TSS -> exon -> intron -> intergenic**

If a read overlaps a TSS \index{transcription start site (TSS)} and an intron, 
...
We will now use the data from **H3K4me3** (Sharp), **H3K36me3** (Broad), and **POL2** (Mixed)
...
### Peak calling - sharp Experiments\index{Peak calling}
...
### Broad regions\index{Peak calling}
...
will be used as input for the motif discovery.

Load the CTCF peaks, and select the top 500 peaks.
Nearby ChIP peaks can have overlapping coordinates.
After selection, overlapping CTCF peaks have to be merged using
the **reduce** function. If we do not execute this step, we will
include the same sequence multiple times in the sequence set, and
artificially enrich DNA patterns.
...
Firstly we load the  **rGADEM** [@rGADEM] package:
...
```{r motif-discovery.seqlogo, fig.cap='Motif with highest enrichment in top 500 CTCF peaks', width = 6, height = 3}
...
3. Use GViz to visualize the profiles for CTCF, SMC3 and ZNF143 [Difficulty: Beginner]
...
How many motifs do you observe? How do the motifs look like (visualize the motif logs)? [Difficulty: Intermediat]
