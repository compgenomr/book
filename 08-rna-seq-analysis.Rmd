...
kable(goResults[order(goResults$p.value), 
                c(3:4, 7, 10, 12)][1:10,],booktabs = TRUE)
```
...
kable(gseaResults$less, 
      booktabs = TRUE,caption = 'Down-regulation statistics')
```

We can see that the random gene set shows no significant up or down-regulation, while the gene set we defined using the top GO term shows a significant up-regulation (adjusted p-value < 0.0007). It is worthwhile to visualize these systematic changes in a heatmap:
