...
The quality control consists of the read quality control and adapter trimming.
...
The following chapter presumes that the user is already familiar with 
...
You should also know what PCR\index{PCR} is, what
...
The available data sets can be listed using the `list.files()` function:
...
The details of clustering methodologies are described in \@ref(unsupervisedLearning).
...
than ChIP of different proteins. The second function is to see whether our experiments conform to known prior knowledge. For example, we would expect to see proteins greater similarity between proteins
...
```{r sample-clustering.tilling_window}
...
```{r sample-clustering.show_tilling_window, include=TRUE, echo=FALSE, eval=TRUE}
tilling_window
```
...
The easiest way to access our data would be to load the .bam files into the browser.This will show us the sequence, and position of every mapped read. If we want to view multiple samples in parallel, loading every mapped read can be restrictive -
...
`Gviz` is a tool which enables exhaustive customized visualization of 
...
The distribution of reads is shown on \@ref(fig:Figure-BrowserScreenshot)
...
Jaccard index is defined as an intersection between two boolean vectors, 
...
We will no shift the coverage vector from the plus strand by 1 - 400 base pairs, and for each pair shift we will calculate the Jaccard index between the vectors
...
We can finally plot the shift in basepairs versus the correlation coefficient: 
...
If all of the windows do not have the same length (i.e. when at the ChIP-seq peaks), then the normalization is a prerequisite.
...
We can now visualize the relationship using  a scatterplot.
Figure \@ref(fig:gc-tidy-plot) compares the GC content dependency on the CPM between
the first and the second CTCF replicate. In this case, the replicates looks similar.
...
1) TSS (transcription start sites)
\index{transcription start site (TSS)}
2) exon 3) intron and 4) intergenic with the following hierarchy: **TSS -> exon -> intron -> intergenic**. This means that If a read overlaps a TSS \index{transcription start site (TSS)} and an intron, 
it will be annotates as TSS. 
...
`AnnotationHub`\index{R Packages!\texttt{AnnotationHub}} is a Bioconductor 
based online resource which contains a large amount of experiments from various
sources. We will use the AnnotationHub to download the location of 
genes corresponding to the **hg38** genome.
...
With **TSS**
\index{transcription start site (TSS)} having the highest priority and the 
intergenic regions having the lowest priority.
...
ChIP experiments to show the differences in the signal profiles. We will use the the bigWig files to visualize the signal profiles around a
...
Figure \@ref(fig:chip-type-plot-gviz) shows the signal profile around the URB1 gene.
...
We will now use `normR` [@helmuth_2016] package for peak calling in sharp and broad peak experiments.
...
We can now repeat the CTCF versus Input plot, and label significantly marked peaks.Using the count overlaps we mark which of our 1kb regions contained significant peaks.
...
Figure \@ref(fig:peak-calling-sharp-peak-calling-plot) shows see that `normR` 
...
Because the sophisticated statistical model, 
...
We can now create a genome browser screenshot around a peak regions.
...
In order to calculate the percentage of CTCF peaks which contain a know CTCF
...
The motifs are usually represented as matrices of 4-by-N dimensions. In the matrix, 4 rows corresponds to one nucleotide (A, C, G, T). 
...
is called a position frequency matrix - PFM\index{position frequency matrix (PFM)}. In some sources, this matrix is also called as "position probability matrix (PPM)". One way to construct such matrices is to get experimentally verified sequences that are bound by the protein of interest and then to use a motif finding algorithm. 
...
sequence by summing up the the base position specific scores we obtain from this log scaled matrix. This matrix is formally called "position specific scoring matrix (PSSM) or position specific weight matrix (PWM). We can use this matrix to scan the genome in a sliding window manner and calculate a score for each window. Usually, a cutoff value is needed to call a motif hit. The higher the score you get from the PWM for a particular sequence the better it is. Traditional algorithms we will use in the following sections use 80% of the maximum rescaled score you can obtain from a PWM as the default cutoff for a hit. The rescaling is simple min-max rescaling where you rescale the score by subtracting the minimum score and dividing that by $max(PWMscore)-min(PWMscore)$. The motif scanning approach is illustrated in Figure \@ref(fig:FigurePWMScanning). In this example, ACACT is not considered a hit because its score only corresponds to only $15.6$ % of the rescaled maximum score.

(ref:FigurePWMScanning) PWM scanning principle. A genomic sequence is scanned by a PWM  matrix. This matrix used to measure how likely is that the transcription factor will bind each nucelotide in each position. Here we are looking at how likely it is that our TF will bind to the sequence ACACT. The score for this sequence is -3.6. The maximal score obtainable by the PWM is 7.2 and minimun is -5.6. After min-max rescaling, -3.6 corresponds to 15% score and ACACT is not considered a hit
...
cumulative binding preference. This tells us how important is each nucleotide  for the binding. It additionally allows us to visually represent the probability matrices as sequence logos.
...
highest quality peaks. In this tutorial we will use `rGADEM`\index{R Packages!\texttt{rGADEM}} 
pacakge for motif discovery.
`rGADEM` is a unsupervised, stochastic motif discovery tools, which uses
...
will be used as input for the motif discovery.  Nearby ChIP peaks can have overlapping coordinates. After selection, overlapping CTCF peaks have to be merged using the `reduce()` function from `GenomicRanges` package. If we do not execute this step, we will include the same sequence multiple times in the sequence set, and artificially enrich DNA patterns.
...
We are now ready to run the motif discovery.
Firstly we load the  `rGADEM` package:
...
`rGADEM` package contains a convenient `plot()` function for
...
The motif show in Figure \@ref(fig:motif-discovery-logo) corresponds to the
...
`opts` parameter defines which `PWM` database to use for comparison.
...
1. Apply the fragment size estimation procedure to all ChIP and Input available data sets [Difficulty: **Beginner**]
...
7. Calculate the cross correlation for the both CTCF replicates, and
...
1. Repeat the motif discovery analysis on peaks from ZNF143 transcription factor.
How many motifs do you observe? How do the motifs look like (visualize the motif logs)? [Difficulty: **Intermediate**]
