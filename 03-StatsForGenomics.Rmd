...
that represents the general properties of the data. The Figure \@ref(fig:pax6ReplicatesChp3) shows a sample of
20 expression values from PAX6 gene.

```{r pax6ReplicatesChp3,fig.align='center',  out.width='50%',echo=FALSE,warning=FALSE,fig.height=5.6,fig.cap="Expression of PAX6 gene in 20 replicate experiments"}
set.seed(1)
old.par <- par()
...
expression value obtained from the experiments. In R, `mean()` function will calculate the \index{mean}
...
```{r pax6MorereplicatesChp3,out.width='75%',fig.width=6.5,echo=FALSE,warning=FALSE,fig.cap="Expression of all possible PAX6 gene expressions measures on all available biological samples (left). Expression of PAX6 gene from statistical sample, a random subset, from the population of biological samples (Right). "}
...
will calculate the mean of a provided vector of numbers.  \index{median}
...
This is called interquartile range \index{interquartile range} , and 
...
```{r boxplot2Chp3,fig.height=5.1,out.width='50%',echo=FALSE,warnings=FALSE,message=FALSE,fig.cap="Boxplot showing 25th percentile and 75th percentile and median for a set of points sample from a normal distribution with mean=6 and standard deviation=0.7"}
...
summarizes them but also they are functions that assigns each outcome of a \index{normal distribution}
...
Below in Figure \@ref(fig:zscore),  we are showing the Z-score and the associated probabilities derived
...
negative binomial distribution. These distributions are used to model count \index{Poisson distribution}
data such as sequencing read counts.

- `df` (F distribution) and `dchisq` (Chi-Squared distribution) are used \index{F distribution}
in relation to distribution of variation. F distribution is used to model  \index{Chi-Squared distribution}
...
estimate the precision of population parameter estimates, is known as the __bootstrap__.\index{bootstrap resampling}
...
of the mean of the sample using bootstrap to build confidence intervals, the resulting plot after this procedure is shown in Figure \@ref(fig:bootstrapChp3).
...
common interval to use, and would \index{confidence interval}
...
The good thing about CLT as long as the sample size is large regardless of \index{central limit theorem (CLT)}
the population distribution, the distribution of sample means drawn from
that population will always be normal. In Figure \@ref(fig:sampleMeanschp3), we are repeatedly 
...
using standard normal distribution, $N(0,1)$, only works when the when we know the \index{normal distribution}
...
sample standard deviation and use something called _t distribution_ instead  \index{t distribution}
...
is simply number of data points minus number of parameters estimated.\index{degrees of freedom} Here
we are estimating the mean from the data and the distribution is for the means, therefore degrees of freedom is $n-1$. The resulting distributions are shown in Figure \@ref(fig:tdistChp3).
```{r tdistChp3,echo=FALSE,warning=FALSE,message=FALSE,out.width='60%',fig.cap="Normal distribution and t distribution with different degrees of freedom. With increasing degrees of freedom, t distribution approximates the normal distribution better."}
...
```{r randomTestchp3,out.width='60%',fig.cap="The null distribution for differences of means obtained via randomization. The original difference is marked via blue line. The red line marks the value that corresponds to P-value of 0.05"}
set.seed(100)
...
We can also calculate the difference between means using a t-test\index{t-test}. Sometimes we will have too few data points in a sample to do meaningful
...
rank,$i$, :$\frac{m \cdot p_i }{i}$, this is derived from  Benjamini–Hochberg \index{P-value}
...
We try to optimize a function, often called "cost function" or "loss function". \index{loss function}
The cost function
...
This is the basis of "gradient descent" algorithm.\index{gradient descent} With the help of partial 
...
derivatives and converge on the bottom of the cost function "well". Such optimization methods is the core of machine learning methods we will cover later in Chapters \@ref(unsupervisedLearning) and
...
We can also think of this problem from more a statistical point of view. In \index{maximum likelihood estimation}
...
, 
you get the following values for $\beta_0$ and $\beta_1$ for simple regression \index{linear regression}
...
`lm()` function, `y~x`, where `y` is the simulated $Y$ values and `x` is the explanatory variables $X$. We will then use `abline()` function to draw the fit. The resulting plot is shown in Figure \@ref(fig:geneExpLinearModel).
```{r geneExpLinearModel,out.width='60%',fig.cap="Gene expression and histone modification score modelled by linear regression"}
mod1=lm(y~x)
...
not exact, with every random sample they will vary. Below in Figure \@ref(fig:regCoeffRandomSamples), we
...
Notice also standard error depends on the spread of $X$. If $X$ values have more \index{Residual Standard Error (RSE)}
...
_  (RSE)_ simply is the square-root of the \index{Residual Standard Error (RSE)}
...
called __"Residual sum of squares"__, RSS. \index{Residual sum of squares (RSS)}So RSE is 
...
the
response and the explanatory variables. _Correlation_ is a general measure of \index{correlation}
linear
...
above called Pearson correlation coefficient. The values that change in a similar manner will have a positive coefficient, the values that change in \index{correlation}
...
the Figure \@ref(fig:CorCovar), we are showing $R^2$, correlation 
coefficient and covariance for different scatter plots.

```{r CorCovar,fig.width=9,fig.height=3,echo=FALSE,warning=FALSE,message=FALSE, fig.cap="Correlation and covariance for different scatter plots"}
set.seed(32)
...
and original response variables ($(cor(Y,\hat{Y}))^2$ ) equals to $R^2$ for \index{$R^2$}
...
cutoff), p-values from coefficient t-tests will be wrong\index{t-test}. In summary, 
...
a linear regression.\index{linear regression} We can sure use t-test for that as well if there 
...
can be encoded in 0 and 1. Below, we are showing linear regression with categorical variable. We then plot the fitted line. This plot is shown in Figure \@ref(fig:LMcategorical).
...
- linear models and derivations of equations including matrix notation
    - Applied Linear Statistical Models by Kutner, Nachtsheim, et al. [@kutner2003applied]
    - Elements of statistical learning by Hastie & Tibshirani [@friedman2001elements]
    - An Introduction to statistical learning by James, Witten, et al.[@james2013introduction]
...
3. simulate 30 random variables using `rpois()` function, do this 1000 times and calculate means of each sample. Plot the sampling distributions of the means
using a histogram. Get the 2.5th and 97.5th percentiles of the
distribution. [Difficulty: **Beginner/Intermediate**] 
4. Use `t.test` function to calculate confidence intervals
of the mean on the first random sample `pois1` simulated from`rpois()` function below.[Difficulty: **Intermediate**] 
...
```
5. Use bootstrap confidence interval for the mean on `pois1`[Difficulty: **Intermediate/Advanced**] 
6. compare the theoretical confidence interval of the mean from `t.test` and the bootstrap confidence interval. Are they similar ?[Difficulty: **Intermediate/Advanced**]  
7. Try to recreate the following figure, which demonstrates the CLT concept.[Difficulty: **Advanced**] 
...
2. Test the difference of means of the following simulated genes
using the randomization, t-test and `wilcox.test()` functions.
Plot the distributions using histograms and boxplots.[Difficulty: **Intermediate/Advanced**] 
...
`gexpFile=system.file("extdata","geneExpMat.rds",package="compGenomRData") data=readRDS(gexpFile)` The data has 100 differentially expressed genes.First 3 columns are the test samples, and the last 3 are the control samples. Do 
...
the p-values. Do a p-value histogram and compare two approaches in terms of the number of significant tests with 0.05 threshold.
...
`hmodFile=system.file("extdata","HistoneModeVSgeneExp.rds",package="compGenomRData")`. There 
...
8. fit the model model for prediction of expression data using:
...
Inspect summary() function output in each case, which terms are significant. [Difficulty:**Beginner/Intermediate**] 
10. Is using H3K4me3 and H3K27me3 better than the model with only H3K4me3. [Difficulty:**Intermediate**] 
11. Plot H3k4me3 vs H3k27me3. Inspect the points that does not
