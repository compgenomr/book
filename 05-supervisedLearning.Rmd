...
### classification 
1. Use no data transformation and do classification. what is the effect on classification accuracy for KNN
2. use bootstrap resampling with caret KNN
3. get variable importance metrics from randomforest and compare that with DALEX

### Regression
1. Split 20% of the methylation-age data as test data and run elastic net regression on the training portion to tune parameters and test it on the test portion.(intermediate difficulty) 
2. Run an ensemble model for regression using **caretEnsemble** or **mlr** package and compare the results with the elastic net and random forest model.
