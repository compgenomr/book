...
You will encounter more with the latter so this is likely what will
...
```{r,RealGene,fig.cap="A) Representation of a gene at UCSC browser. Boxes indicate exons, and lines indicate introns. B) Partial sequence of FATE1 gene as shown in NCBI GenBank database.",fig.align = 'center',out.width='60%',ref.label='RealGene',echo=FALSE}
...
```{r,regSummary,fig.cap="Representation of regulatory regions in animal genomes",fig.align = 'center',out.width='60%',ref.label='regSummary',echo=FALSE}
...
small-interfering RNAs (siRNAs), microRNAs (miRNAs), promoter-associated RNAs
...
long ncRNAs originating from promoters of coding genes. snoRNAs are also shown
...
Human and chimpanzee genomes are 98.8% similar. The 1.2% difference is what separates
...
applications in the clinic are also gaining momentum: there are already diagnostic
...
- _Where do RNA-binding proteins bind?_
...
for the DNA fragments that are bound by the protein of interest. This part 
depends on available molecular biology and chemistry techniques, and the final 
...
```{r,HTassays,fig.cap="Common steps of High-throughput assays in genome biology",fig.align = 'center',out.width='60%',ref.label='HTassays',echo=FALSE}
...
on the library preperation protocol different strategies are applied for analysis. A sequencing library is composed of fragments of RNA or DNA ready to be sequenced. The library preparation primarily depends on the experiment of interest. There are a number of library preparation protocols aimed at quantifying different signals from the genome. Some of the potential analysis strategies for different library-prep protocols and processed output of read alignments
...
a base position in a read mismatches the base in the genome. Read aligners 
...
```{r,HTseq,fig.cap="High-throughput sequencing summary",fig.align = 'center',out.width='70%',ref.label='HTseq',echo=FALSE}
...
the additional data sets is a necessary step to develop hypothesis and understand
the data.
...
not available through genome browsers. Normally, every high-throughput data set
...
http://www.ebi.ac.uk/ena. These repositories accept high-throughput data sets 
...
dedicated to certain genome biology or disease related problems and 
