...
There are multiple sources of genomic annotation. **UCSC**\index{UCSC Genome Browse}, 
**Genbank**, and **Ensembl**\index{Ensembl} databases represent stable resources, 
...
ENSEMBL\index{ENSEMBL} gene annotation.
...
CTCF motif.
The figure \@ref(fig:peak-quality-seqLogo-plot) shows which are the most
...
```{r peak-quality-seqLogo-plot, echo=FALSE, include=TRUE, fig.cap="CTCF sequence motif visualized as a sequence logo. Y axis ranges from zero to two, and corresponds to the amount of information each base in the motif contributes to the overall motif. The larger the letter the greater the probability of observing just one defined base on the designated position.", fig.width=6, fig.height=3}
